...
| 面向 | Claude Design | Claude Code CLI |
|---|---|---|
| **初次建立** | ✅ Onboarding 時讀 codebase + design files，**自動建立 design system**（colors / typography / components）並套到後續所有新專案 | 需設計一套方式（讀 repo / Figma MCP / submodule） |
| **後續更新** | 手動流程：Organization settings → 找到 design system → Open → **Remix**（聊天式更新）。原始檔變動**不會自動反映** | `cross-team-submod/local-cache-ttl/`——TTL 到期自動 fetch guideline |
