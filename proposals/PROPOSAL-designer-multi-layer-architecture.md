...
Local Cache + TTL 工具包已實作完成，下一步是：

**統一寫好一份引用架構，做成 code snippets to add in hooks and CLAUDE.md**
...
```
Admin 維護統一的「hooks + CLAUDE.md snippets」
                    ↓
Designer 直接複製貼上（或執行 setup script）
                    ↓
自動引用正確的 Company/Dept 規範
```

### 預期產出

1. **標準化的 hooks 配置** — 寫好可直接用的 `settings.json` snippet
2. **標準化的 sources.json** — 預填好 Company/Dept 的 raw URL
3. **一鍵 setup script** — Designer 執行一次即完成所有設定
